...
def generate_reasoning(data, future_preds_scaled, scaler,
    ...
    # ── Combined signal ───────────────────────────────────────────────────────
    # News is the dominant signal — LSTM only provides a small directional hint
    combined = (lstm_signal * 0.10) + (tech_signal * 0.25) + (news_score * 0.65)
    ...
    if mom30 is not None:
        mom30_capped = max(-15.0, min(15.0, mom30))
        daily_trend   = mom30_capped / 30.0
        # RSI mean-reversion logic:
        # Oversold (RSI<35): stock likely to bounce → DAMPEN negative momentum
        # Overbought (RSI>65): stock likely to pull back → DAMPEN positive momentum
        if rsi and rsi > 65 and daily_trend > 0:
            daily_trend *= 0.55   # slow overbought rise
        elif rsi and rsi < 35 and daily_trend < 0:
            daily_trend *= 0.40   # strong dampen: oversold → mean reversion expected
        elif rsi and rsi < 35 and daily_trend > 0:
            daily_trend *= 1.15   # already recovering from oversold: boost slightly
        # News sentiment is the primary forward-looking driver
        daily_trend += news_score * 1.4
        proj_30d_pct  = daily_trend * 30
        proj_price    = last_close * (1 + proj_30d_pct / 100)
    else:
        proj_30d_pct = news_score * 1.4 * 30
        proj_price   = last_close * (1 + proj_30d_pct / 100)
    ...
    # ── Summary ───────────────────────────────────────────────────────────────
    summary = (
        f"Combined signal (LSTM 5d direction 10% + Technical 25% + News Sentiment 65%) is "
        f"\"{verdict.upper()}\". Momentum-based 30-day price estimate: {change_str} "
        f"(~${proj_price:.2f}) — news sentiment is the primary driver of this projection."
    )
...
def get_prediction():
    ...
    try:
        ...
        # Dynamically fit a light Ridge regression model for sliding-window forecasting
        model = Ridge(alpha=1.0)
        # x_data is shape (samples, time_steps, features). sklearn needs 2D (samples, features)
        x_data_2d = x_data.reshape(x_data.shape[0], -1) 
        model.fit(x_data_2d, y_data)
        
        predictions = model.predict(x_data_2d)
        ...
        for _ in range(30):
            current_batch_2d = current_batch.reshape(1, -1)
            nxt = model.predict(current_batch_2d)
            future_preds_scaled.append(nxt[0])
            # Reshape nxt to (1,1,1) for array concatenation
            nxt_3d = np.array(nxt[0]).reshape(1, 1, 1)
            current_batch = np.append(current_batch[:, 1:, :], nxt_3d, axis=1)
            
        future_preds_scaled_arr = np.array(future_preds_scaled).reshape(-1, 1)   # shape (30, 1)
        ...
        # ── News sentiment (Fetch early to influence projections) ────────────
        ...
        if len(close_series) >= 30:
            ...
            mom30_capped = max(-15.0, min(15.0, mom30_raw))
            daily_rate = mom30_capped / 30.0
            ...
            # RSI mean-reversion logic:
            # Oversold (RSI<30): stock likely to bounce → DAMPEN negative downtrend
            # Overbought (RSI>70): stock likely to pull back → DAMPEN positive uptrend
            if rsi_val > 70 and daily_rate > 0:
                daily_rate *= 0.50   # slow overbought rally
            elif rsi_val < 30 and daily_rate < 0:
                daily_rate *= 0.40   # strongly dampen: oversold → expect mean reversion
            elif rsi_val < 30 and daily_rate > 0:
                daily_rate *= 1.15   # recovering from oversold: small boost

            # News sentiment is the primary forward-looking driver
            daily_rate += (news_score * 1.4)
        else:
            daily_rate = (news_score * 1.4) if news_score else 0.0

        mom_proj = np.array([last_close * (1 + daily_rate / 100 * (t + 1)) for t in range(30)])

        # ── Dampen LSTM prediction drift ──
        # Compound error can cause LSTM predictions to diverge wildly. Cap the total 
        # deviation from the last close to ±15%.
        lstm_pcts = (inv_future_raw - last_close) / last_close
        
        # Inject News Sentiment Bias directly into LSTM output.
        # Raised to 0.50 so bullish/bearish news meaningfully tilts the curve.
        sentiment_shift = np.linspace(0, news_score * 0.50, 30)
        lstm_pcts += sentiment_shift
        
        lstm_pcts_damped = np.clip(lstm_pcts, -0.15, 0.15)
        # Flatten the arrays to 1D to prevent broadcasting mismatch with mom_proj
        inv_future_raw_damped = last_close * (1 + lstm_pcts_damped.flatten())

        # LSTM weight decays from 0.20 → 0.00 over 30 days.
        # By day 30 the forecast is 100% momentum+news driven.
        lstm_weights = np.linspace(0.20, 0.00, 30)
        mom_weights  = 1.0 - lstm_weights
        inv_future   = lstm_weights * inv_future_raw_damped + mom_weights * mom_proj
...
def model_info():
    ...
    return jsonify({
        ...
        'training': 'Dynamic (Real-time)',
        'description': 'Model is trained instantly on sliding windows during the prediction request to ensure low memory footprint on Render.'
    })


@app.route('/api/model-debug', methods=['GET'])
def model_debug():
    """Deep debug: now simplified for scikit-learn."""
    return jsonify({'status': 'TensorFlow layers introspection removed.'})
